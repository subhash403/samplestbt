# `press_and_wait` and the other functions in this file will be provided by
# stb-tester in the next release. This file can be removed when stb-tester v29
# is released.
...
def wait_for_transition_to_end(
    ...
    :param stbt.Frame initial_frame: The frame of video when the transition
        started. If `None`, we'll grab a new frame from the device under test.
        For short transitions there is a race condition if you don't specify the
        initial frame -- the transition may have already finished before we grab
        the initial frame!
