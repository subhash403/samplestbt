...
def test_modify_DVR_options_10plus_times():
    ...
    for _ in range(10):
        ...
        sleep(2)
        stbt.press('KEY_RECORD')
        if not stbt.wait_until(lambda: stbt.match("images/dvr/edit_ep_rec.png")) and not stbt.wait_until(
                lambda: stbt.match("images/dvr/edit_ep_deleted.png")):
            for _ in " " * 2: stbt.press('KEY_EXIT')
            sleep(2)
            stbt.press('KEY_RECORD')
...
def test_tuning_channels_5_times():
    ...
    if not stbt.wait_until(lambda: stbt.match("images/env/do_you_want_to_upgrade.png")) and not stbt.wait_until(
            lambda: stbt.match("images/env/channel_unavailable.png")) and not stbt.wait_until(
            lambda: stbt.match("images/env/title_blocked.png")):
